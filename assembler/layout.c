...
#define MARGIN 12
#define TERMINATOR_SIZE 36
#define TERMINATOR2_SIZE 20
#define BLOCK_SIZE 120
#define CABLE_WIDTH 20
#define TURNS_BAR_HEIGHT 70
#define MSG_BAR_HEIGHT 70
struct block{ //16
	int dimension[4];
};
struct map{
	struct block blocks[9]; // 16 * 9
	struct block answer[9]; // 16 * 9
	int total_turns; // 4
	int default_blank_location; // 4
	int answer_blank_location;// 4
	int entry_block; // 4
	int entry_direction; // 4
	int exit_block; // 4
	int exit_direction; // 4
};

extern GtkWidget *window; //32
extern GtkWidget *draw_area;  //32
extern struct map maps[5]; // 1580
extern int blank_location; // 4
extern int remain_turns; // 4
extern char remain_str[5]; // 5
extern char level_str[5]; // 5
extern gboolean mask; // 4
extern gboolean current_level_done; // 4
extern int current_level; // 4
extern int mask_x; // 4
extern int mask_y; // 4
extern int mask_width; // 4
...
void layout_init(){
	...
	gtk_window_set_title(GTK_WINDOW(window), "Plumber");
